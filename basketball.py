...
# CTRL+A on Windows and run the Jupyter cell
...
goalAccuracyPercentage = np.matrix.round((FieldGoals / FieldGoalAttempts) * 100)

print(goalsPerGame)
print(goalsPerGame[Pdict["LeBronJames"], Sdict["2007"]])
print(minutesPlayedPerGame)
print(goalAccuracyPercentage)

# Visualization ----------------------------------
# We can also use short form for the line below,
# c for color, ls for line style, lw for line width
# marker -> s for squire & o for circle, ms for marker size
# Refer to matplotlib Documentation

plt.rcParams['figure.figsize'] = 10, 6
plt.title("Payers' Salary")

plt.plot(Salary[0], label=Players[0], color="Green", linestyle="-", linewidth=1.5, marker="o", markersize=7.5, alpha=1)
plt.plot(Salary[1], label=Players[1], color="Black", linestyle="-", linewidth=1.5, marker="o", markersize=7.5, alpha=1)
plt.plot(Salary[2], label=Players[2], color="Blue", linestyle="-", linewidth=1.5, marker="o", markersize=7.5, alpha=1)
plt.plot(Salary[4], label=Players[4], color="Red", linestyle="-", linewidth=1.5, marker="o", markersize=7.5, alpha=1)
...
# -------------------------------------------------------------------------------------
# ----------- We can make this stuff more easy and clear by using Function ------------
# -------------------------------------------------------------------------------------


def myPlot(data, playerList=Players):
    plt.title("Payers' Games")
    plt.rcParams['figure.figsize'] = 10, 6

    Color = {"KobeBryant": "#9c27b0", "JoeJohnson": "#f44336", "LeBronJames": "#673ab7", "CarmeloAnthony": "#E91E63",
             "DwightHoward": "#2196F3", "ChrisBosh": "#009688", "ChrisPaul": "#8bc34a", "KevinDurant": "#ffc107",
             "DerrickRose": "#795548", "DwayneWade": "#607d8b"}

    for name in playerList:
        plt.plot(data[Pdict[name]], label=name, color=Color[name], linestyle="-", linewidth=1.5,
                 marker="o",
...
# Using specified players from playerList
myPlot(Points, ["KobeBryant", "JoeJohnson", "LeBronJames", "CarmeloAnthony"])

# Using Default Parameters playerList=Players calls all the players of the list if not anyone specified
